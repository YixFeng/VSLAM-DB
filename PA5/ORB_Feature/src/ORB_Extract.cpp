...
// global variables
string first_file = "/home/fengyixiao/Dark_Blue/SLAM/course/Complete/PA5/ORB_Feature/1.png";
string second_file = "/home/fengyixiao/Dark_Blue/SLAM/course/Complete/PA5/ORB_Feature/2.png";
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        for(int dx = -half_patch_size; dx < half_patch_size; dx++)
            ...
            {
                uchar pixel = image.at<uchar>(kp.pt.y + dy, kp.pt.x + dx); //行,列
                m10 += pixel * dx;
                m01 += pixel * dy;
            }
    ...
    cout << "bad_points:" << bad_points << endl;
...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern
int ORB_pattern[256 * 4] = {        // "一维" 的随机点组，用于计算描述子
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    ...
    for (auto &kp: keypoints) {
        ...
        DescType d(256, false); //false是vector<bool>长度为256的初始化值

        for (int i = 0; i < 256; i++) {
            cv::Point2f p(ORB_pattern[i * 4], ORB_pattern[i * 4 + 1]);
            cv::Point2f q(ORB_pattern[i * 4 + 2], ORB_pattern[i * 4 + 3]);

            float sin_theta = sin(kp.angle * pi / 180);
            float cos_theta = cos(kp.angle * pi / 180);
            cv::Point2f pp = cv::Point2f(p.x * cos_theta - p.y * sin_theta + kp.pt.x, p.y * cos_theta + p.x * sin_theta + kp.pt.y);
            cv::Point2f qq = cv::Point2f(q.x * cos_theta - q.y * sin_theta + kp.pt.x, q.y * cos_theta + q.x * sin_theta + kp.pt.y);
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i1 = 0; i1 < desc1.size(); i1++)
    {
        ...
        for(int i2 = 0; i2 < desc2.size(); i2++)
        {
            ...
            {
                if(desc1[i1][index] != desc2[i2][index])
                    distance ++;
            }
